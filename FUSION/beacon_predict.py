...
#
# add non tle based recordings to schedule
#
care_mode = False
import care
...
#                    print time.ctime(risetime)
    ...
    def writeFile(self,fname,year,month,day,ddir):
        f = open(fname, 'w')
            ...
            for pi in range(0,min(len(self.risings[si]),len(self.settings[si]))):
                ...
                f.write("%1.10f %1.10f %1.6f %1.6f %1.3f %1.3f %1.3f %s\n" %
                        (self.risings[si][pi],
                         ...
                         self.name[si]))
        if care_mode:
            care.write_care(f,year=year,month=month,day=day,i0=sat_idx,ddir=ddir)
        f.close()
...
def write_pass_files(datestr="2010/4/30 10:00:00",
                     ...
                     altitude=197.03,
                     sat_elev_cutoff=10.0, year=2015, month=9, day=5):
    ...
    sats.writeFile("%s/passes.txt"%(ddir),year,month,day,ddir)

if __name__ == "__main__":
    parser = optparse.OptionParser()

    parser.add_option('-y', '--year', dest='year', type="int", default=2016)
    parser.add_option('-m', '--month', dest='month', type="int", default=1)
    parser.add_option('-n', '--day', dest='day', type="int", default=16)
    parser.add_option('-d', '--dir', dest='dir', type="string", default="/home/jmgomezs/Documents/beacon20")
    #parser.add_option('-d', '--dir', dest='dir', type="string", default="Test")
    (op, args) = parser.parse_args()
    dstr = "%04d/%02d/%02d 00:00:00"%(op.year,op.month,op.day)
    lat = "67:21:51.0"
    lon = "26:37:45.0"
    alt = 197.03 
    cutoff = 10.0
    write_pass_files(dstr,op.dir,lat,lon,alt,cutoff,op.year,op.month,op.day)
